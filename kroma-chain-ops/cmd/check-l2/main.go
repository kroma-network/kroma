...
func checkProtocolVault(addr common.Address, client *ethclient.Client) error {
	contract, err := bindings.NewProtocolVault(addr, client)
	if err != nil {
		return err
	}
	recipient, err := contract.RECIPIENT(&bind.CallOpts{})
	...
	log.Info("ProtocolVault", "RECIPIENT", recipient.Hex())
	if recipient == (common.Address{}) {
		return errors.New("RECIPIENT should not be address(0)")
	}
	...
	log.Info("ProtocolVault", "MIN_WITHDRAWAL_AMOUNT", minWithdrawalAmount)

	version, err := contract.Version(&bind.CallOpts{})
	if err != nil {
		return err
	}
	log.Info("ProtocolVault version", "version", version)
	return nil
}
...
func checkKromaMintableERC721Factory(addr common.Address, client *ethclient.Client) error {
	contract, err := bindings.NewKromaMintableERC721Factory(addr, client)
	if err != nil {
		return err
	}
	...
	log.Info("KromaMintableERC721Factory", "BRIDGE", bridge.Hex())
	if bridge == (common.Address{}) {
		return errors.New("KromaMintableERC721Factory.BRIDGE is zero address")
	}
	...
	log.Info("KromaMintableERC721Factory", "REMOTE_CHAIN_ID", remoteChainID)

	version, err := contract.Version(&bind.CallOpts{})
	if err != nil {
		return err
	}
	log.Info("KromaMintableERC721Factory version", "version", version)
	return nil
}
...
func checkKromaMintableERC20Factory(addr common.Address, client *ethclient.Client) error {
	contract, err := bindings.NewKromaMintableERC20Factory(addr, client)
	if err != nil {
		return err
	}
	...
	log.Info("KromaMintableERC20Factory", "BRIDGE", bridgeLegacy.Hex())
	if bridgeLegacy == (common.Address{}) {
		return errors.New("KromaMintableERC20Factory.BRIDGE is zero address")
	}
	...
	log.Info("KromaMintableERC20Factory", "bridge", bridge.Hex())

	initialized, err := getInitialized("KromaMintableERC20Factory", addr, client)
	if err != nil {
		return err
	}
	log.Info("KromaMintableERC20Factory", "_initialized", initialized)

	initializing, err := getInitializing("KromaMintableERC20Factory", addr, client)
	if err != nil {
		return err
	}
	log.Info("KromaMintableERC20Factory", "_initializing", initializing)

	version, err := contract.Version(&bind.CallOpts{})
	if err != nil {
		return err
	}
	log.Info("KromaMintableERC20Factory version", "version", version)
	return nil
}

func checkValidatorRewardVault(addr common.Address, client *ethclient.Client) error {
	contract, err := bindings.NewValidatorRewardVault(addr, client)
	if err != nil {
		return err
	}
	recipient, err := contract.RECIPIENT(&bind.CallOpts{})
	...
	log.Info("ValidatorRewardVault", "RECIPIENT", recipient.Hex())
	if recipient == (common.Address{}) {
		return errors.New("RECIPIENT should not be address(0)")
	}
	...
	log.Info("ValidatorRewardVault", "MIN_WITHDRAWAL_AMOUNT", minWithdrawalAmount)

	version, err := contract.Version(&bind.CallOpts{})
	if err != nil {
		return err
	}
	log.Info("ValidatorRewardVault version", "version", version)
	return nil
}
