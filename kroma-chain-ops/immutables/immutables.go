...
func l2ImmutableDeployer(backend *backends.SimulatedBackend, opts *bind.TransactOpts, deployment deployer.Constructor) (*types.Transaction, error) {
	...
	case "MintManager":
		governanceToken, ok := deployment.Args[0].(common.Address)
		if !ok {
			return nil, fmt.Errorf("invalid type for governanceToken")
		}
		...
		_, tx, _, err = bindings.DeployMintManager(opts, backend, governanceToken, initMintPerBlock, slidingWindowBlocks, decayingFactor)
	// [Kroma: END]
	default:
