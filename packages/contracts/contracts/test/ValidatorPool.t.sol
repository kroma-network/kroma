...
contract ValidatorPoolTest is L2OutputOracle_Initializer {
    ...
    function test_constructor_succeeds() external {
        ...
        assertEq(pool.VALIDATOR_HARDFORK_BLOCK(), validatorHardforkBlock);
...
contract ValidatorPool_ValidatorHardfork_Test is L2OutputOracle_ValidatorHardfork_Initializer {
    MockL2OutputOracle mockOracle;
    ...
    function test_deposit_afterValidatorHardfork_reverts() external {
        vm.warp(oracle.computeL2Timestamp(pool.VALIDATOR_HARDFORK_BLOCK()));
        vm.prank(trusted);
        vm.expectRevert(
    ...
    function test_createBond_prepareValidatorHardfork_succeeds() external {
        vm.prank(trusted);
        ...
        for (uint256 i; i <= poolLastOutputIndex; i++) {
            ...
            if (i < firstUnbondOutputIndex || i >= beforeValidatorHardforkLastOutputIndex) {
                // check if next priority validator is zero address before first unbond or after validator hardfork
                assertTrue(mockPool.getNextPriorityValidator() == address(0));
            } else {
                // check if next priority validator is set after first unbond and before validator hardfork
                assertTrue(mockPool.getNextPriorityValidator() != address(0));
            }
        }
    }
    ...
    function test_isTerminated_succeeds() external {
        ...
        poolTerminated = pool.isTerminated(beforeValidatorHardforkLastOutputIndex);
        assertFalse(poolTerminated);
