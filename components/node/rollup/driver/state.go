...
func (s *Driver) eventLoop() {
	...
	for {
		...
		if s.driverConfig.ProposerEnabled && !s.driverConfig.ProposerStopped &&
			...
			if s.driverConfig.ProposerMaxSafeLag > 0 && s.derivation.SafeL2Head().Number+s.driverConfig.ProposerMaxSafeLag <= s.derivation.UnsafeL2Head().Number {
			...
			} else if s.proposer.BuildingOnto().ID() != s.derivation.UnsafeL2Head().ID() {
