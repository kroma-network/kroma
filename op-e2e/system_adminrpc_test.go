...
func TestPostUnsafePayload(t *testing.T) {
	...
	require.NoError(t, err)
	payload, err := eth.BlockAsPayload(blockNumberOne, sys.RollupConfig.CanyonTime, sys.RollupConfig.KromaMPTTime)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	payload, err = eth.BlockAsPayload(blockNumberTwo, sys.RollupConfig.CanyonTime, sys.RollupConfig.KromaMPTTime)
	require.NoError(t, err)
