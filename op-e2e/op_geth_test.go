...
func TestPreregolith(t *testing.T) {
	t.Skip("kroma does not support pre-regolith")
	InitParallel(t)
...
func TestRegolith(t *testing.T) {
	...
	for _, test := range tests {
		...
		t.Run("ReturnUnusedGasToPool_"+test.name, func(t *testing.T) {
			if test.regolithTime != hexutil.Uint64(0) {
				t.Skip("kroma does not support pre-regolith")
			}
		...
		t.Run("RejectSystemTx_"+test.name, func(t *testing.T) {
			t.Skip("IsSystemTransaction does not exist in the kroma geth")
			...
			systemTx, err := derive.L1InfoDeposit(1, opGeth.L1Head, opGeth.SystemConfig, true)
			// [Kroma: START]
			// systemTx.IsSystemTransaction = true
			// [Kroma: END]
			require.NoError(t, err)
