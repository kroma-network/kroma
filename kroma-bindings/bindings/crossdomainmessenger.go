...
func (it *CrossDomainMessengerPausedIterator) Next() bool {
	...
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(CrossDomainMessengerPaused)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
...
func (_CrossDomainMessenger *CrossDomainMessengerFilterer) FilterPaused(opts *bind.FilterOpts) (*CrossDomainMessengerPausedIterator, error) {
	...
	logs, sub, err := _CrossDomainMessenger.contract.FilterLogs(opts, "Paused")
	if err != nil {
		return nil, err
	}
	return &CrossDomainMessengerPausedIterator{contract: _CrossDomainMessenger.contract, event: "Paused", logs: logs, sub: sub}, nil
}
...
func (_CrossDomainMessenger *CrossDomainMessengerFilterer) WatchPaused(opts *bind.WatchOpts, sink chan<- *CrossDomainMessengerPaused) (event.Subscription, error) {
	...
	logs, sub, err := _CrossDomainMessenger.contract.WatchLogs(opts, "Paused")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(CrossDomainMessengerPaused)
				if err := _CrossDomainMessenger.contract.UnpackLog(event, "Paused", log); err != nil {
					return err
				}
				event.Raw = log
...
func (_CrossDomainMessenger *CrossDomainMessengerFilterer) FilterSentMessage(opts *bind.FilterOpts, target []common.Address, sender []common.Address) (*CrossDomainMessengerSentMessageIterator, error) {
	...
	logs, sub, err := _CrossDomainMessenger.contract.FilterLogs(opts, "SentMessage", targetRule, senderRule)
	if err != nil {
		return nil, err
	}
...
func (_CrossDomainMessenger *CrossDomainMessengerFilterer) WatchSentMessage(opts *bind.WatchOpts, sink chan<- *CrossDomainMessengerSentMessage, target []common.Address, sender []common.Address) (event.Subscription, error) {
	...
	logs, sub, err := _CrossDomainMessenger.contract.WatchLogs(opts, "SentMessage", targetRule, senderRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
...
func (it *CrossDomainMessengerUnpausedIterator) Next() bool {
	...
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(CrossDomainMessengerUnpaused)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
...
func (_CrossDomainMessenger *CrossDomainMessengerFilterer) FilterUnpaused(opts *bind.FilterOpts) (*CrossDomainMessengerUnpausedIterator, error) {
	...
	logs, sub, err := _CrossDomainMessenger.contract.FilterLogs(opts, "Unpaused")
	if err != nil {
		return nil, err
	}
	return &CrossDomainMessengerUnpausedIterator{contract: _CrossDomainMessenger.contract, event: "Unpaused", logs: logs, sub: sub}, nil
}
...
func (_CrossDomainMessenger *CrossDomainMessengerFilterer) WatchUnpaused(opts *bind.WatchOpts, sink chan<- *CrossDomainMessengerUnpaused) (event.Subscription, error) {
	...
	logs, sub, err := _CrossDomainMessenger.contract.WatchLogs(opts, "Unpaused")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(CrossDomainMessengerUnpaused)
				if err := _CrossDomainMessenger.contract.UnpackLog(event, "Unpaused", log); err != nil {
					return err
				}
				event.Raw = log
