...
func TestL2ProposerRPCDepositTx(t *testing.T) {
	...
	cancel()
	require.Error(t, err, "a DepositTx was accepted by L2 syncer over RPC when it should not have been.")
}
...
func TestMixedDepositValidity(t *testing.T) {
	...
	for i := 0; i < len(testAccounts); i++ {
		...
		ctx, cancel = context.WithTimeout(context.Background(), txTimeoutDuration)
		startL2Balance, err := l2Sync.BalanceAt(ctx, testAccount.L2Opts.From, nil)
		cancel()
		require.NoError(t, err)

		// Obtain the transactor's starting nonce on L2.
		ctx, cancel = context.WithTimeout(context.Background(), txTimeoutDuration)
		startL2Nonce, err := l2Sync.NonceAt(ctx, testAccount.L2Opts.From, nil)
		cancel()
		require.NoError(t, err)
	...
	for _, transactor := range transactors {
		...
		// Obtain the L2 syncer account balance
		ctx, cancel = context.WithTimeout(context.Background(), txTimeoutDuration)
		endL2SyncBalance, err := l2Sync.BalanceAt(ctx, transactor.Account.L2Opts.From, nil)
		cancel()
		require.NoError(t, err)

		// Obtain the L2 syncer account nonce
		ctx, cancel = context.WithTimeout(context.Background(), txTimeoutDuration)
		endL2SyncNonce, err := l2Sync.NonceAt(ctx, transactor.Account.L2Opts.From, nil)
		cancel()
		require.NoError(t, err)
...
func TestMixedWithdrawalValidity(t *testing.T) {
	...
	for i := 0; i <= 8; i++ {
		...
		t.Run(fmt.Sprintf("withdrawal test#%d", i+1), func(t *testing.T) {
			...
			// Obtain the transactor's starting balance on L2.
			ctx, cancel = context.WithTimeout(context.Background(), txTimeoutDuration)
			transactor.ExpectedL2Balance, err = l2Sync.BalanceAt(ctx, transactor.Account.L2Opts.From, nil)
			cancel()
			require.NoError(t, err)
			...
			// Obtain the L2 syncer account balance
			ctx, cancel = context.WithTimeout(context.Background(), txTimeoutDuration)
			endL2SyncBalance, err := l2Sync.BalanceAt(ctx, transactor.Account.L1Opts.From, nil)
			cancel()
			require.NoError(t, err)

			// Obtain the L2 syncer account nonce
			ctx, cancel = context.WithTimeout(context.Background(), txTimeoutDuration)
			endL2SyncNonce, err := l2Sync.NonceAt(ctx, transactor.Account.L1Opts.From, nil)
			cancel()
			require.NoError(t, err)
