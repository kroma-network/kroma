...
func TestBatcherKeyRotation(gt *testing.T) {
	...
	proposer.ActL2PipelineFull(t)
	syncer.ActL2PipelineFull(t)
	require.Equal(t, uint64(2), proposer.L2Safe().L1Origin.Number, "l2 chain with new L1 origins")
	require.Equal(t, proposer.L2Safe(), syncer.L2Safe(), "fully synced syncer")
	...
	payload, err := engCl.PayloadByNumber(t.Ctx(), proposer.L2Safe().Number+12) // 12 new L2 blocks: 5 with origin before L1 block with batch, 6 with origin of L1 block with batch, 1 with new origin that changed the batcher
	require.NoError(t, err)
	...
	proposer.ActL2PipelineFull(t)
	syncer.ActL2PipelineFull(t)
	require.Equal(t, proposer.L2Safe().L1Origin.Number, uint64(4), "safe l2 chain with two new l1 blocks")
	require.Equal(t, proposer.L2Safe(), syncer.L2Safe(), "fully synced syncer")

	// now try to build a new L1 block, and corresponding L2 blocks, and submit with the old batcher
	before := proposer.L2Safe()
	miner.ActEmptyBlock(t)
	proposer.ActL1HeadSignal(t)
	proposer.ActBuildToL1Head(t)
	batcherA.ActSubmitAll(t)
	...
	proposer.ActL2PipelineFull(t)
	syncer.ActL2PipelineFull(t)
	require.Equal(t, proposer.L2Safe(), before, "no new safe l1 chain")
	require.Equal(t, syncer.L2Safe(), before, "syncer is ignoring old batcher too")
	...
	proposer.ActL2PipelineFull(t)
	syncer.ActL2PipelineFull(t)
	require.NotEqual(t, proposer.L2Safe(), before, "new safe l1 chain")
	require.NotEqual(t, syncer.L2Safe(), before, "syncer is not ignoring new batcher")
	...
	proposer.ActL2PipelineFull(t)
	syncer.ActL2PipelineFull(t)
	require.Equal(t, uint64(2), proposer.L2Safe().L1Origin.Number, "l2 safe is first batch submission with original batcher")
	require.Equal(t, uint64(3), proposer.L2Unsafe().L1Origin.Number, "l2 unsafe l1 origin is the block that included the first batch")
	require.Equal(t, proposer.L2Safe(), syncer.L2Safe(), "syncer safe head check")
	require.Equal(t, proposer.L2Unsafe(), syncer.L2Unsafe(), "syncer unsafe head check")
	...
	proposer.ActL2PipelineFull(t)
	syncer.ActL2PipelineFull(t)
	require.Equal(t, proposer.L2Safe(), proposer.L2Unsafe(), "all L2 blocks are safe now")
	require.Equal(t, proposer.L2Unsafe(), syncer.L2Unsafe(), "syncer synced")
	...
	proposer.ActL2PipelineFull(t)
	syncer.ActL2PipelineFull(t)
	require.Equal(t, uint64(3+6+1), syncer.L2Safe().L1Origin.Number, "sync new L1 chain, while key change is reorged out")
	require.Equal(t, proposer.L2Unsafe(), syncer.L2Unsafe(), "syncer synced")
}
...
// the gas limit change event. And checks if a syncer node can reproduce the same gas limit change.
