...
func TestBatcherKeyRotation(gt *testing.T) {
	...
	require.Equal(t, sequencer.L2Safe(), syncer.L2Safe(), "fully synced syncer")
	...
	require.Equal(t, sequencer.L2Safe(), syncer.L2Safe(), "syncer safe head check")
	require.Equal(t, sequencer.L2Unsafe(), syncer.L2Unsafe(), "syncer unsafe head check")
	...
	require.Equal(t, sequencer.L2Unsafe(), syncer.L2Unsafe(), "syncer synced")
...
func TestGasLimitChange(gt *testing.T) {
	...
	require.Equal(t, sequencer.L2Unsafe(), syncer.L2Safe(), "syncer stays in sync, even with gaslimit changes")
}
...
func TestValidatorRewardScalarChange(gt *testing.T) {
	...
	require.Equal(t, sequencer.L2Unsafe(), syncer.L2Safe(), "syncer stays in sync, even with validator reward scalar changes")
}
