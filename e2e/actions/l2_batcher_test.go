...
func TestBatcher(gt *testing.T) {
	...
	syncCl := syncEngine.EthClient()
	vTx, isPending, err := syncCl.TransactionByHash(t.Ctx(), tx.Hash())
	require.NoError(t, err)
...
// and channel-decoding syncer code in the derive package.
