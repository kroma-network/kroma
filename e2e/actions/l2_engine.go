...
type L2Engine struct {
	...
	l2ForceEmpty     bool                      // when no additional txs may be processed (i.e. when proposer drift runs out)
...
func (e *L2Engine) ActL2IncludeTx(from common.Address) Action {
	return func(t Testing) {
		...
		if e.l2ForceEmpty {
			...
			// t.InvalidAction("cannot include any proposer txs")
			return
		}
		...
		receipt, err := core.ApplyTransaction(e.l2Cfg.Config, e.l2Chain, &common.Address{},
