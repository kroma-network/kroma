...
def main():
    ...
    ops_bedrock_dir = pjoin(monorepo_dir, 'ops-devnet-migration')
    ...
    paths = Bunch(
      ...
      mpt_genesis_l2_path=pjoin(devnet_dir, 'mpt-genesis-l2.json'),
...
def devnet_deploy(paths):
    ...
    if os.path.exists(paths.mpt_genesis_l2_path):
      ...
      log.info('Generating MPT L2 genesis and rollup configs.')
      run_command([
        'go', 'run', 'cmd/main.go', 'genesis', 'l2',
        '--l1-rpc', 'http://localhost:8545',
        '--deploy-config', paths.devnet_config_path,
        '--l1-deployments', paths.addresses_json_path,
        '--outfile.l2', paths.mpt_genesis_l2_path,
    ...
    log.info('Bringing up `kroma-node`, `kroma-batcher`.')
    run_command(['docker', 'compose', 'up', '-d', 'kroma-node', 'kroma-batcher'], cwd=paths.ops_bedrock_dir, env={
        'PWD': paths.ops_bedrock_dir
    })

    run_command(['docker', 'compose', '--profile', 'mpt-fullnode', 'up', '-d', ], cwd=paths.ops_bedrock_dir, env={
      'PWD': paths.ops_bedrock_dir
    })

    run_command(['docker', 'compose', '--profile', 'zkt-fullnode', 'up', '-d', ], cwd=paths.ops_bedrock_dir, env={
        'PWD': paths.ops_bedrock_dir
    })
