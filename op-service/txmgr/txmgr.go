...
func (m *SimpleTxManager) queryReceipt(ctx context.Context, txHash common.Hash, sendState *SendState) *types.Receipt {
	...
	m.l.Debug("Transaction mined, checking confirmations", "tx", txHash,
		...
		"numConfirmations", m.Config.NumConfirmations)
...
func (m *SimpleTxManager) increaseGasPrice(ctx context.Context, tx *types.Transaction) (*types.Transaction, error) {
	...
	ctx, cancel := context.WithTimeout(ctx, m.Config.NetworkTimeout)
	defer cancel()
	signedTx, err := m.Config.Signer(ctx, m.Config.From, newTx)
	if err != nil {
...
func (m *SimpleTxManager) SuggestGasPriceCaps(ctx context.Context) (*big.Int, *big.Int, *big.Int, error) {
	...
	if minTipCap := m.Config.MinTipCap; minTipCap != nil && tip.Cmp(minTipCap) == -1 {
		m.l.Debug("Enforcing min tip cap", "minTipCap", m.Config.MinTipCap, "origTipCap", tip)
		tip = new(big.Int).Set(m.Config.MinTipCap)
	}
	if minBaseFee := m.Config.MinBaseFee; minBaseFee != nil && baseFee.Cmp(minBaseFee) == -1 {
		m.l.Debug("Enforcing min base fee", "minBaseFee", m.Config.MinBaseFee, "origBaseFee", baseFee)
		baseFee = new(big.Int).Set(m.Config.MinBaseFee)
	}
...
func (m *SimpleTxManager) checkBlobFeeLimits(blobBaseFee, bumpedBlobFee *big.Int) error {
	...
	if thr := m.Config.FeeLimitThreshold; thr != nil && thr.Cmp(bumpedBlobFee) == 1 {
		return nil
	}
	maxBlobFee := new(big.Int).Mul(calcBlobFeeCap(blobBaseFee), big.NewInt(int64(m.Config.FeeLimitMultiplier)))
	if bumpedBlobFee.Cmp(maxBlobFee) > 0 {
		return fmt.Errorf(
			"bumped blob fee %v is over %dx multiple of the suggested value: %w",
			bumpedBlobFee, m.Config.FeeLimitMultiplier, ErrBlobFeeLimit)
	}
	return nil
}
